...
void solution()
{
    ...
    int k, s;
    in3(n, k, s);
    //Sum of the n odd numbers is:- n^2
    int new_sum = pow(n, 2);
    //Calculating the difference b/w the sums 
    int diff = s - new_sum;
    //Since the number of terms has been reduced by one 
    //Hence the total number of occurrences is reduced by one = (k-1)
    int ans = diff / (k - 1);
    //Number can be calculated by dividing the differences by the number of occurrences 
    out(ans);
    line;
}
/************************************/
